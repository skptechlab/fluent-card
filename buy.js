...
// Supabase functions through API
...
document.getElementById('connectWalletBtn').onclick = async () => {
  if ('solana' in window) {
    ...
    try {
      ...
      document.getElementById('balance').textContent = `$MASQ Balance: ${balance}`;
      if (balance >= 1_000) document.getElementById('buyPackBtn').style.display = 'inline-block';

      const profile = await getUserProfile();
      const ownedSets = profile.owned_sets || [];
      const username = profile.username || 'Not set';
      const linkedWallet = profile.wallet || 'None linked';
      document.getElementById('ownedSets').textContent = `Sets owned: ${ownedSets.join(', ') || 'None'}`;
      document.getElementById('userInfo').innerHTML = `<p><strong>Username:</strong> ${username}</p><p><strong>Linked Wallet:</strong> ${linkedWallet}</p>`;
    } catch (err) {
      console.error("Error connecting to wallet:", err);
      alert('Failed to connect wallet.');
    }
...
document.getElementById('buyPackBtn').onclick = async () => {
  if (!provider || !userPublicKey) return alert('Please connect your wallet first!');
  const profile = await getUserProfile();
  const ownedSets = profile.owned_sets || [];
  const walletInDB = profile.wallet;
  const connectedWallet = userPublicKey.toBase58();
  ...
  if (walletInDB && walletInDB !== connectedWallet) {
    return alert("This wallet does not match your linked wallet. Contact support to update it.");
  } else if (!walletInDB) {
    ...
    if (confirmLink) {
      ...
      return alert("Wallet not linked. Purchase cancelled.");
    }
  }
  ...
  const transferIx = createTransferInstruction(
    userTokenAccount, treasuryTokenAccount, userPublicKey,
    1_000, [], TOKEN_PROGRAM_ID
  );
  instructions.push(transferIx);
